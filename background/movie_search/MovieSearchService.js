var srtPlayer = srtPlayer || {};
if (typeof exports !== 'undefined') {
    exports.srtPlayer = srtPlayer;
    srtPlayer.Descriptor = require('../../descriptor/Descriptor.js').srtPlayer.Descriptor;
    srtPlayer.Redux = require('../../redux/redux').srtPlayer.Redux;
    srtPlayer.ActionCreators = require('../../redux/actionCreators').srtPlayer.ActionCreators;
}

srtPlayer.MovieSearchService = srtPlayer.MovieSearchService || ((fetch = window.fetch) => {

        let previousQuery = srtPlayer.Redux.getState().movieSearch.query;
        let unsubscribe = srtPlayer.Redux.subscribe(() => {
            let movieSearch = srtPlayer.Redux.getState().movieSearch;
            if (previousQuery !== movieSearch.query && movieSearch.query !== "") {
                previousQuery = movieSearch.query;
                loadData(movieSearch.query);
            }
        });

        const FALLBACK_POSTER_IMAGE = "data:image/png;base64,";


        async function loadData(query) {
            try {
                const response = await fetch('https://app.plus-sub.com/movie/search/' + decodeURIComponent(query));

                if (response.status !== 200) {
                    srtPlayer.Redux.dispatch(srtPlayer.ActionCreators.setMovieSearchResult({
                        message:`Failed to search movie. Status (${response.status})`,
                        src:"movieSearchService"
                    },true));

                    return;
                }
                const result = await getMoreInformationFor(createImbdInformationFrom(await response.json()));
                srtPlayer.Redux.dispatch(srtPlayer.ActionCreators.setMovieSearchResult(result));
            } catch (err) {
                srtPlayer.Redux.dispatch(srtPlayer.ActionCreators.setMovieSearchResult({
                    message:`Failed to search movie. Are you Disconnected? Err: (${err})`,
                    src:"movieSearchService"
                },true));
            }
        }

        function createImbdInformationFrom(imdbResponse) {
            return Object.keys(imdbResponse).map((k) => imdbResponse[k]).reduce((p, c) => {
                p.push(...c);
                return p;
            }, []);
        }

        async function getMoreInformationFor(imdbResult) {

            const maxRequestedImdb = 10;
            const imdbResultPartial = imdbResult.slice(0, imdbResult.length > maxRequestedImdb ? maxRequestedImdb : imdbResult.length);

            return Promise.all(imdbResultPartial.map(async rawImdb => {
                try {

                    let response = await fetch('https://app.plus-sub.com/movie/information/' + rawImdb.id)
                    if(response.status !== 200){
                       return generateRawImdbFallback(rawImdb);
                    }

                    let result = await response.json();
                    var newEntry;
                    if (result.movie_results && result.movie_results.length) {
                        newEntry = {
                            imdbID: rawImdb.id,
                            Title: rawImdb.title,
                            Year: result.movie_results[0].release_date,
                            Rating: '-',
                            Genre: '-',
                            Country: '-',
                            Poster: result.movie_results[0].poster_path ? 'https://image.tmdb.org/t/p/w500' + result.movie_results[0].poster_path : FALLBACK_POSTER_IMAGE
                        };
                    } else if (result.tv_results && result.tv_results.length) {
                        newEntry = {
                            imdbID: rawImdb.id,
                            Title: rawImdb.title,
                            Year: result.tv_results[0].first_air_date,
                            Rating: '-',
                            Genre: '-',
                            Country: result.tv_results[0].origin_country,
                            Poster: result.tv_results[0].poster_path ? 'https://image.tmdb.org/t/p/w500' + result.tv_results[0].poster_path : FALLBACK_POSTER_IMAGE
                        };
                    } else {
                        newEntry = {
                            imdbID: rawImdb.id,
                            Title: rawImdb.title,
                            Year: '-',
                            Rating: '-',
                            Genre: '-',
                            Country: '-',
                            Poster: FALLBACK_POSTER_IMAGE
                        };
                    }

                    return Object.assign(newEntry, {valueField: JSON.stringify(newEntry)});
                } catch (err) {
                    return generateRawImdbFallback(rawImdb);
                }
            }));
        }

        function generateRawImdbFallback(rawImdb){
            return {
                imdbID: rawImdb.id,
                Title: rawImdb.title,
                Year: '-',
                Rating: '-',
                Genre: '-',
                Country: '-',
                Poster: FALLBACK_POSTER_IMAGE
            };
        }
        return {
            shutdown:unsubscribe
        }
    });

//instant service does not correct initialize messageBus (in testfiles)
if (typeof exports === 'undefined' && typeof srtPlayer.MovieSearchService === 'function') {
    srtPlayer.MovieSearchService = srtPlayer.MovieSearchService();
}
